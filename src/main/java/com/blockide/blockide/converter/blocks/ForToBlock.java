...
public class ForToBlock extends Block{
    ...
    public ForToBlock(BlockModel model) {
        super(model);
        ...
        if (loverBound == null) {
            throw new IllegalArgumentException("Lover bound is required for For to block");
        }
        if (upperBound == null) {
            throw new IllegalArgumentException("Upper bound is required for For to block");
        }
        if (body == null) {
            throw new IllegalArgumentException("Body is required for For to block");
        }
    }

    @Override
    public String generateCode() {
        return String.format("for (int i = %s; i < %s; ++i) {\n%s}", loverBound, upperBound, body);
    }
}
